<template>
  <div>
    ...
    <div id="mytable">
      ...
      >
        ...
        <el-table-column
          ...
          width="100vw"
        ></el-table-column>
         <el-table-column prop="updateTime" label="报警时间"></el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              ...
              size="mini"
              @click="handleFirst(scope.row.id)"
    ...
    >
      <el-form
        ref="carAlarmFormRef"
        :rules="carAlarmFormRules"
        :model="carAlarmForm"
        :inline="true"
      >


        <el-form-item label="请确认是？" prop="status">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          <el-radio-group v-model="carAlarmForm.status">
            <el-radio :label="1">正常报警</el-radio>
        ...
        <el-form-item>
          ...
          <el-button @click="resetCarAlarmForm">取消</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
    ...
    >
      ...
      >
        ...
        <el-form-item label="请确认这些都是？" prop="status">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          <el-radio-group v-model="carAlarmForm2.status">
            <el-radio :label="1">正常报警</el-radio>
            <el-radio :label="0">误报</el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item>
          <el-button type="primary" @click="handleMore">确认</el-button>
          <el-button @click="resetCarAlarmForm2">取消</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
  ...
  methods: {
    ...
    resetCarInfoForm2() {
     ...
     handleFirst(id){
        ...
        // console.log(this.id);
          ...
          this.carAlarmDialog=true;
          this.CarAlarmId=this.allData[index].id;
          //
          // console.log(window.sessionStorage.usern);
        //  console.log(this.CarAlarmId,this.carAlarmForm.remarks,this.carAlarmForm.status,window.sessionStorage.namen);
    ...
    handle() {
      ...
      // });
       // 表单验证
      let isLegal = true;
      this.$refs["carAlarmFormRef"].validate(valid => {
        if (!valid) {
          this.$message({
            message: "请确认是正常报警还是误报！",
            type: "warning",
            duration: 2000
          });
          isLegal = false;
      ...
      let status=2;
      if (this.carAlarmForm.status === "误报") {
        status = 0;
      }
      if (this.carAlarmForm.status === "正常报警") {
        status = 1;
      }
     ...
     // console.log("11111" + "  " + this.allData[index].handled);
        updateCarAlarmData(this.CarAlarmId,this.carAlarmForm.remarks,this.carAlarmForm.status,this.username)
          .then(() => {
            this.$message({
              message: "处理成功！",
              type: "success",
    ...
    handlePage() {
      ...
      for (const iter of this.multipleSelection) {
        ...
        // this.CarAlarmId=this.allData[index].id;
        // this.status=this.allData[index].status;
        listData.push(iter.id);
        // console.log(listData);

        // console.log(listData.length);
      if(listData.length!=0){
          this.carAlarmDialog2=true;
      };
      }
     ...
     handleMore(){
      ...
      for (const iter of this.multipleSelection) {
        ...
        listData.push(iter.id);
           ...
           let isLegal = true;
      this.$refs["carAlarmFormRef2"].validate(valid => {
        if (!valid) {
          this.$message({
            message: "请确认是正常报警还是误报！",
            type: "warning",
      ...
      let status=2;
      if (this.carAlarmForm2.status === "误报") {
        status = 0;
      }
      if (this.carAlarmForm2.status === "正常报警") {
        status = 1;
      };



          pageHandle(listData,this.carAlarmForm2.remarks,this.carAlarmForm2.status,window.sessionStorage.namen).then(() => {
          // q.success("成功")
          this.$message({
            message: "处理成功！",
            type: "success",
    ...
    insertData() {
      ...
      let data = {
        id: this.nextTodoId++,
        terminalId: this.carAlarmForm.terminalId,
        latitude: this.carAlarmForm.latitude,
        longitude: this.carAlarmForm.longitude,
        handled: 0,
    ...
    // 点击取消和X按钮，重置表单
    resetCarAlarmForm() {
      //this.$refs.carAlarmFormRef.resetFields();
      this.$refs.carAlarmFormRef.resetFields();
      this.carAlarmDialog=false;
      // this.carAlarmDialog2=false;
    },
     resetCarAlarmForm2() {
      ...
      this.$refs.carAlarmFormRef2.resetFields();
